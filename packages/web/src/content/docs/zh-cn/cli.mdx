...
| `OPENCODE_MODELS_FETCH_TIMEOUT_MS`    | number  | 模型目录首次拉取的等待上限（默认 `3000`） |
| `OPENCODE_OFFLINE`                    | boolean | 跳过所有尽力而为的联网动作：模型目录拉取、更新检查、LSP 与依赖下载 |
| `OPENCODE_OFFICECLI_PATH`             | string  | 内置 `officecli` MCP 服务器使用的 OfficeCLI 二进制路径 |
| `OPENCODE_DISABLE_OFFICECLI`          | boolean | 关闭内置的 `officecli` MCP 服务器                |
