...
| `OPENCODE_MODELS_FETCH_TIMEOUT_MS`    | number  | Cold-start budget for the provider catalog fetch (default `3000`) |
| `OPENCODE_OFFLINE`                    | boolean | Skip best-effort network calls: catalog fetch, update check, LSP and package downloads |
| `OPENCODE_OFFICECLI_PATH`             | string  | Path to the OfficeCLI binary used by the bundled `officecli` MCP server |
| `OPENCODE_DISABLE_OFFICECLI`          | boolean | Disable the bundled `officecli` MCP server |
